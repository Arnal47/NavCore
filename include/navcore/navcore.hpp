...
struct MatchResult { Edge matched_edge; std::string osm_way_id; std::string road_name; double snapped_latitude{}; double snapped_longitude{}; double lateral_distance_m{}; double heading_difference_deg{}; double score{}; };
RoadGraph load_csv_map(const std::string& path); RoadGraph load_osm_map(const std::string& path); RouteResult route(const RoadGraph&,const std::string&,const std::string&,const RouteOptions& options = RouteOptions{}); double haversine_m(const Node&,const Node&); std::string edge_key(const Edge&); double default_speed_kph(const std::string& highway); std::vector<GpsPoint> load_gps_trace(const std::string& path); MatchResult match_point(const RoadGraph&,const GpsPoint&); std::vector<MatchResult> match_sequence(const RoadGraph&,const std::vector<GpsPoint>&);
}