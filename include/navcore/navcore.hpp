...
#include <vector>
namespace navcore { class SpatialIndex;
...
enum class CandidateBackend { BruteForce, SpatialIndex }; struct GpsPoint { double latitude{}; double longitude{}; std::optional<double> heading_deg; };
...
RoadGraph load_csv_map(const std::string& path); RoadGraph load_osm_map(const std::string& path); RouteResult route(const RoadGraph&,const std::string&,const std::string&,const RouteOptions& options = RouteOptions{}); double haversine_m(const Node&,const Node&); std::string edge_key(const Edge&); double default_speed_kph(const std::string& highway); std::vector<GpsPoint> load_gps_trace(const std::string& path); MatchResult match_point(const RoadGraph&,const GpsPoint&); MatchResult match_point_backend(const RoadGraph&,const GpsPoint&,CandidateBackend,const SpatialIndex* index=nullptr); std::vector<MatchResult> match_sequence(const RoadGraph&,const std::vector<GpsPoint>&); RouteResult route_from_matches(const RoadGraph&,const std::vector<MatchResult>&,const RouteOptions& options = RouteOptions{});
}