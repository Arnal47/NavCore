...
RouteResult route(const RoadGraph& g, const std::string& from, const std::string& to, const RouteOptions& opt) {
  ...
  for(const auto& pair:g.nodes()) for(const auto& e:g.outgoing(pair.first)) { const double geo=haversine_m(g.node(e.from),g.node(e.to)); if(geo>0.0) { const double cost=opt.cost==Cost::Distance?e.length_m:e.travel_time_s; scale=std::min(scale,cost/geo); } }
